/// This is a Work in progress, it allows for a multiple of COST to be accepted and remits a single mint URI token.
/// TODO - remit multiple URI tokens based on num_uris count.

#include "hookapi.h"
#include <stdint.h>

#define GUARD(maxiter) _g(__LINE__, (maxiter)+1)
#define MAX_URIS 5 // Maximum number of URIs to mint in one transaction
uint64_t num_uris = 0;
...
// HookOn: Incoming Payment Gateway and URI Token Mint
if (tt == 00) {
    ...
    // Check if the hook has a COST
    if (hasCost <= 0) {
        rollback(SBUF("Error: This hook is missing a COST aka URI token sell price!"), __LINE__);
    }
    ...
    uint64_t cost_xah = UINT64_FROM_BUF(cbuf);
    uint64_t cost_drops = cost_xah * 1000000; // Convert XAH to drops
    TRACEVAR(cost_drops);

    // Calculate number of URIs to mint
    num_uris = otxn_drops / cost_drops;
    if (num_uris == 0) {
        rollback(SBUF("Error: Payment amount is less than COST!"), __LINE__);
    }
    if (otxn_drops % cost_drops != 0) {
        rollback(SBUF("Error: Payment amount must be an exact multiple of COST!"), __LINE__);
    }
    if (num_uris > count) {
        TRACEVAR(num_uris);
        TRACEVAR(count);
        rollback(SBUF("Error: Insufficient tokens available!"), __LINE__);
    }
    if (num_uris > MAX_URIS) {
        rollback(SBUF("Error: Cannot mint more than 5 URI tokens at a time!"), __LINE__);
    }
