...
int64_t hook(uint32_t reserved) {
    ...
    // Prepare issued amount in XFL (use the exact XAH received)
    uint64_t issued_amount = otxn_drops / 1000000; // XAH, integer part only
    if (issued_amount == 0)
        NOPE("IssueEqual: Issued amount is zero.");
    TRACEVAR(issued_amount);

    // Use default XPN currency
    uint8_t currency[20];
    ...
    if (util_keylet(SBUF(keylet), KEYLET_LINE, SBUF(hook_acc), SBUF(otxn_acc), SBUF(currency)) != 34)
        NOPE("IssueEqual: Could not generate trustline keylet.");
    ...
    if (float_sto(amt_out - 1, 49, SBUF(currency), SBUF(hook_acc), amount_xfl, sfAmount) < 0)
        NOPE("IssueEqual: Failed to serialize issued amount.");
    ...
    if (emit(SBUF(emithash), SBUF(txn)) != 32)
        NOPE("IssueEqual: Failed to emit transaction.");

    DONE("IssueEqual: XPN issued for exact XAH received.");
