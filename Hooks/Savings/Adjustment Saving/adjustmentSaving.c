//**************************************************************
// Genesis Mint Savings Hook - HandyHooks Example
// Author: @Handy_4ndy
//
// Description:
//   Automatically forwards Genesis Mint balance adjustments to up to 3
...
//
// Features:
//   - Forwards Genesis Mint (ttGENESIS_MINT) adjustments to savings accounts
...
#define DONE(x) accept(SBUF("GMS:: Success :: " x), __LINE__)
#define WARN(x) rollback(SBUF("GMS:: Misconfigured :: " x), __LINE__)
#define NOPE(x) rollback(SBUF("GMS:: Error :: " x), __LINE__)
...
{
    TRACESTR("GMS :: Genesis Mint Savings :: Called");
    ...
    if (tt == ttGENESIS_MINT)
    {
        ...
        DONE("Genesis Mint forwarded to savings");
    }
